...
def get_users():
    ...
    if search:
        # Use a parameterized query with bound parameters to prevent SQL injection
        query = text("SELECT * FROM users WHERE username LIKE :pattern OR email LIKE :pattern")
